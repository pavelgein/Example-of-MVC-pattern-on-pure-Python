...
class Router(object):
    ...
    def route(self, request_path, request_get_data, request_cookie):

        if request_cookie != '':
            sessionId = getSessionId(request_cookie)
            if sessions.existsSession(sessionId):
                if not sessions.goodSession(sessionId):
                    return self.badResponse(sessionId)
            else:
                sessionId = sessions.AddNewSession({'num' : 0, 'auth' : False})
        else:
            sessionId = sessions.AddNewSession({'num' : 0, 'auth' : False})

        responseCookie = SimpleCookie()
        responseCookie['id'] = sessionId
        if request_path in self._paths:
            res = self._paths[request_path](request_get_data)
            sessions.UpdateInfo(sessionId, 'num', sessions.GetInfo(sessionId, 'num') + 1)

        else:
            res = self.default_response(request_get_data)

        return res[0], res[1], responseCookie

    def register(self, path, callback):
        self._paths[path] = callback

    def default_response(self, *args):
        return 404, "Nooo 404!", SimpleCookie()

    def badResponse(self, sessionId):
        responseCookie = SimpleCookie()
        responseCookie['id'] = sessionId
        return 200, "Can't show this page!", responseCookie
...
def application(environ, start_response):
    ...
    try:
        request_cookie = environ["HTTP_COOKIE"]
    except KeyError:
        request_cookie = ''

    # TODO: You can add this interesting line
    # print(parse_http_post_data(environ))
    #print(type(request_cookie))

    http_status_code, response_body, response_cookie  = router.route(request_path, request_get_data, request_cookie)
